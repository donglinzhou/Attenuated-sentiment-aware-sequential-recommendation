...
from model.ASSR.ASSR_model_English import ASSR
import torch
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hidden_units', default=40, type=int)
    ...
    file_name = 'ASSR' + '.dataset={}.maxlen={}.hidden={}.head={}'
    file_name = file_name.format(args.dataset, args.maxlen, args.hidden_units,args.num_heads)
    ...
    # 完成数据集切分
    dataset = ASSR_data_partition_English(args.dataset)
    ...
    sampler = WarpSampler_English([user_train,  vpos_train, pos_train, neu_train,  neg_train, vneg_train, time_train],
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        for step in range(num_batch):
            ...
            u, seq, pos, neg, vpos_seq, pos_seq, neu_seq, neg_seq, vneg_seq, time_seq = sampler.next_batch()
            u, seq, pos, neg, vpos_seq, pos_seq, neu_seq, neg_seq, vneg_seq, time_seq = np.array(u), np.array(seq), np.array(pos), np.array(neg), \
            np.array(vpos_seq), np.array(pos_seq), np.array(neu_seq), np.array(neg_seq),  np.array(vneg_seq), np.array(time_seq)

            pos_logits, neg_logits = model(u, seq, vpos_seq, pos_seq, neu_seq, neg_seq, vneg_seq, time_seq, pos, neg)
        ...
        if epoch % 20 == 0:
            ...
            t_test = ASSR_evaluate_test_English(model, dataset, args)            # 评估测试集
            t_valid = ASSR_evaluate_valid_English(model, dataset, args)     # 评估验证集
